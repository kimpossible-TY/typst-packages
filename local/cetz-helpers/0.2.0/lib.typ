...
// Sample a parametrized curve, including both endpoints. Returns coordinates
// only; callers own the CeTZ line style, labels, and mathematical interpretation.
#let sample-curve(point, start, end, segments: 60) = {
  assert(type(segments) == int and segments > 0, message: "segments must be a positive integer")
  range(0, segments + 1).map(i => point(start + i * (end - start) / segments))
}

#let sample-function(f, start, end, segments: 60) = {
  sample-curve(x => (x, f(x)), start, end, segments: segments)
}
